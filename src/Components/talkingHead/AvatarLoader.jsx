...
const AvatarLoader = forwardRef((props, ref) => {
  ...
  const visemeMap = {
    'sil': { mouthOpen: 0, jawOpen: 0 },
    'PP': { mouthOpen: 0, jawOpen: 0 },          // p, b, m
    'FF': { mouthOpen: 0.2, jawOpen: 0.1 },      // f, v
    'TH': { mouthOpen: 0.3, jawOpen: 0.15 },     // th
    'DD': { mouthOpen: 0.3, jawOpen: 0.2 },      // d, t, n
    'kk': { mouthOpen: 0.3, jawOpen: 0.25 },     // k, g
    'CH': { mouthOpen: 0.4, jawOpen: 0.2 },      // ch, j, sh
    'SS': { mouthOpen: 0.2, jawOpen: 0.1 },      // s, z
    'nn': { mouthOpen: 0.2, jawOpen: 0.15 },     // n, l
    'RR': { mouthOpen: 0.3, jawOpen: 0.2 },      // r
    'aa': { mouthOpen: 0.7, jawOpen: 0.6 },      // a (as in father)
    'E': { mouthOpen: 0.4, jawOpen: 0.3 },       // e (as in bed)
    'I': { mouthOpen: 0.3, jawOpen: 0.2 },       // i (as in bit)
    'O': { mouthOpen: 0.5, jawOpen: 0.4 },       // o (as in bought)
    'U': { mouthOpen: 0.3, jawOpen: 0.2 }        // u (as in book)
  };
  ...
  const playAudioWithLipSync = async (audioUrl) => {
    ...
    try {
      ...
      console.log('Audio started playing with enhanced lip sync');
  ...
  useImperativeHandle(ref, () => ({
    speakWithLipSync: playAudioWithLipSync
  }));
  ...
  useEffect(() => {
    ...
    controls.target.set(0, 1.7, 0); // Look at face level
    controls.update();
    ...
    // Update mouth morphs based on audio - SIMPLIFIED VERSION
    const updateLipSync = () => {
      ...
      const volume = Math.min(average / 100, 1); // Normalize to 0-1
      ...
      // Try both formats: with and without 'viseme_' prefix
      const setMorph = (name, value) => {
        // Try with viseme_ prefix
